...
- **ThreatModel** — `Skill("ThreatModel", "<verb> ...")` (STRIDE / MAESTRO / DataFlowDiagram / AbuseCases / SecurityISCs — produces threat tables and atomic security ISCs ready to drop into the calling ISA's `## Criteria`. Counts when invoked for design-time analysis, not just for boilerplate threat lists)
...
The closed list grew from 19 to 20 entries in v6.4.0 with the addition of **ThreatModel**. ThreatModel is auto-included at PLAN for E3+ coding tasks (see "Shift-Left Security Doctrine" below). It contributes to the thinking floor when invoked analytically — for boilerplate threat lists without ISA integration, treat it as a delegation invocation instead and do not double-count.
...
- **ThreatModel** (NEW v6.4.0) — auto-include at PLAN for E3/E4/E5 when the task is coding-shaped (touches code that will run in production OR handles user data OR exposes a network surface). Invoke STRIDE workflow for traditional apps, MAESTRO workflow for agentic-AI work, both if hybrid.
- **Silas at PLAN** (NEW v6.4.0) — auto-include at PLAN for E4/E5 coding tasks (in addition to its existing VERIFY presence). Spawned at PLAN for "identify attack surface before we build" rather than only at VERIFY for "find what we missed."
...
### Shift-Left Security Doctrine (NEW v6.4.0)

**Security is not a VERIFY-phase concern.** Per v6.3.0 and earlier, security entered the Algorithm only at VERIFY (Silas, Cato, RedTeam, security-review skill, pr-review-toolkit). v6.4.0 elevates security to a cross-phase concern that fires at OBSERVE, PLAN, EXECUTE, and VERIFY:

| Phase | Security mechanism | Trigger |
|-------|-------------------|---------|
| **SessionStart (pre-OBSERVE)** | `claude_secure_coder` SessionStart hook loads OWASP Top 10:2025, CWE Top 25, OWASP AST10 (2026), and per-language secure-coding rules into context. | Every session, automatic. |
| **OBSERVE — Security ISC scaffolding** | When the task is coding-shaped, the ISA skill's Scaffold workflow auto-generates security-category ISCs (input validation, authn, authz, secrets handling, crypto, deserialization, dependency hygiene, rate limiting). Each maps to an OWASP / CWE / AST10 category by reference. | E2+ coding tasks. Soft floor of 6 security ISCs at E3, 12 at E4, 24 at E5. |
| **PLAN — Threat modeling** | `Skill("ThreatModel", ...)` invoked before BUILD: STRIDE for traditional apps, MAESTRO for agentic AI, both if hybrid. Output ISCs append to `## Criteria` via `Skill("ISA", "append criteria ...")`. Silas spawned at E4+ for "identify attack surface before we build" pass. | E3+ coding tasks. |
| **EXECUTE — IDE-style feedback** | `claude_secure_coder` PreToolUse `VulnPatternHook` matches deterministic vulnerability patterns (literal API keys, eval/exec on user input, weak crypto) — hard-blocks on literal-secret detection, advises on dangerous APIs. PostToolUse `SAST` hook runs semgrep + trufflehog on the diff with regenerate-loop on HIGH severity (max 2 iterations per file). | Every Edit/Write/MultiEdit, automatic. |
| **VERIFY — Backstop review** | Existing tooling: Silas, Cato (E4/E5), RedTeam (E4+), security-review skill, pr-review-toolkit. Catches residual issues, not primary defense. | E2+ at appropriate tiers. |

**Coding-shaped detection:** a task is coding-shaped if it modifies code that will run in production, handles user-supplied data, exposes a network surface, manipulates the file system on behalf of a user, or implements an AI agent / skill / hook / MCP server. Pure documentation, content authoring, or analysis tasks are NOT coding-shaped and skip the security auto-include.

**Security ISC categories (suggested at OBSERVE for coding-shaped E2+ tasks):**

| Category | Example ISC | OWASP / CWE / AST10 |
|----------|-------------|---------------------|
| Input validation | `ISC-N: every API endpoint validates request body via a typed parser before reaching business logic` | A03 / CWE-20 / CWE-89 |
| Authentication | `ISC-N: passwords stored as argon2id hashes; bcrypt cost ≥12` | A07 / CWE-916 |
| Session lifecycle | `ISC-N: Anti: after logout the prior JWT cannot access /api/me` | A07 / CWE-613 |
| Authorization | `ISC-N: every endpoint requiring auth has explicit object-level authorization check` | A01 / CWE-862 / CWE-285 |
| Secrets handling | `ISC-N: Anti: no literal secrets in source — all from env or secret manager` | A02 / CWE-798 |
| Crypto | `ISC-N: no MD5 or SHA1 in password or signing paths; AES-GCM for symmetric encryption` | A02 / CWE-327 |
| Deserialization | `ISC-N: Anti: yaml.load (without SafeLoader), pickle.loads, marshal.loads on untrusted data` | A08 / CWE-502 |
| Path / command injection | `ISC-N: Anti: shell=True with user input; subprocess always array-form` | A03 / CWE-78 / CWE-22 |
| Rate limiting | `ISC-N: auth endpoints rate-limited per-IP and per-account; default 10/minute` | A07 / CWE-799 |
| Dependency hygiene | `ISC-N: lockfile present; CI runs npm audit / pip-audit; HIGH+ blocks merge` | A06 / CWE-1104 |
| Logging | `ISC-N: Anti: no secrets, raw PII, or full request bodies in logs` | A09 / CWE-532 |
| AST (agentic) | `ISC-N: Anti: tool output parsed as instructions without user confirmation` | AST01 / AST02 |

These templates live in `~/.claude/skills/ThreatModel/Workflows/SecurityISCs.md` and are rendered into the ISA at OBSERVE when the task is coding-shaped. Refine via the Splitting Test like any other ISC.

**Why this exists** (provenance): security previously surfaced only at VERIFY when fixes are 10-100x more expensive than at design or write time. v6.4.0 inverts this: SessionStart context, design-time threat modeling, write-time SAST + secret detection, with the existing review tooling as backstop. Pattern matches Anthropic's published Semgrep plugin (PostToolUse + regenerate-loop), Phoenix Security's open kit, and the OWASP shift-left consensus circa 2025-2026.
